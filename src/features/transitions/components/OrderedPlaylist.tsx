    ...
    <div className='ordered-playlist'>
      <div className='playlist-header'>
        ...
        <div className='playlist-meta'>
          ...
          <span className='ordering-info'>Ordered by {getMetricLabel(playlist.orderingMetric)}</span>
        </div>
      </div>
        ...
        {playlist.songs.map((song, index) => (
          <div key={`${song.id}-${index}`} className='playlist-song-item'>
            ...
            <div className='metric-value'>
              ...
              <span className='metric-number'>{getMetricValue(song, playlist.orderingMetric)}</span>
            </div>

            <div className='transition-indicator'>
              {index < playlist.songs.length - 1 && <div className='flow-arrow'>→</div>}
            </div>
          </div>
        ))}
      </div>
              ...
              {playlist.songs.length > 0 &&
                (() => {
                  const sum = playlist.songs.reduce((acc, song) => {
                    const value =
                      playlist.orderingMetric === 'tempo'
                        ? song.tempo
                        : playlist.orderingMetric === 'energy'
                        ? song.energy
                        : playlist.orderingMetric === 'valence'
                        ? song.valence
                        : playlist.orderingMetric === 'danceability'
                        ? song.danceability
                        : playlist.orderingMetric === 'loudness'
                        ? song.loudness
                        : song.popularity;
                    return acc + value;
                  }, 0);
            ...
            <span className='insight-value'>
              {(() => {
                ...
                return genres.length <= 3
                  ? genres.join(', ')
                  : `${genres.slice(0, 2).join(', ')} +${genres.length - 2} more`;
              })()}
            </span>
          </div>
        </div>
